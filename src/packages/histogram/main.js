    ...
    if (metrics.length === 1) {
      seriesItem.itemStyle = {
        color: function (params) {
          return params.dataIndex % 2 === 0 ? extra.color[1] : extra.color[0]
        }
      }
    }
...
export const histogram = (dimensions, source, settings, extra) => {
  ...
  let legend = Object.assign({}, { show: metrics.length > 1, type: 'scroll' }, extra.legend, { data: targets })
...
export const bar = (dimensions, source, settings, extra) => {
  ...
  const series = getBarSeries({ metrics, targets, sources, showLine, stack, label, extra })
  const options = { grid, legend, tooltip, xAxis, yAxis, series }
  return options
}
